(async function collectSystemInfo() {
  ...
  // --- Brave / anti-fingerprint ---
  async function detectBraveAPI() {
    try { if(navigator.brave && typeof navigator.brave.isBrave==='function'){return {available:true,isBrave:!!(await navigator.brave.isBrave())}} } catch{} 
  ...
  function detectPluginsMasking(){try{const plugins=navigator.plugins?Array.from(navigator.plugins).map(p=>p.name):[];const empty=plugins.length===0;const platformLikelyDesktop=/win|mac|linux/i.test(navigator.platform||'');return{plugins,empty,platformLikelyDesktop,masked:empty&&platformLikelyDesktop}}catch{return{plugins:[],empty:true,masked:false}}}
  async function canvasFingerprintHash(){try{const c=document.createElement('canvas');c.width=400;c.height=60;const ctx=c.getContext('2d');ctx.textBaseline='top';ctx.font='16px Arial';ctx.fillStyle='#f60';ctx.fillRect(0,0,400,60);ctx.fillStyle='#069';ctx.fillText('Brave-FP-Test-'+navigator.userAgent,2,2);ctx.fillStyle='rgba(102,204,0,0.7)';ctx.fillText('𝕏𝕐𝕫 ☃ 漢',2,22);const d=ctx.getImageData(0,0,400,60).data;if(crypto?.subtle?.digest){const buf=new Uint8Array(d.buffer);const hashBuf=await crypto.subtle.digest('SHA-256',buf);return Array.from(new Uint8Array(hashBuf)).map(b=>b.toString(16).padStart(2,'0')).join('');}let h=2166136261>>>0;for(let i=0;i<d.length;i++){h^=d[i];h=Math.imul(h,16777619)>>>0;}return'fnh-'+h.toString(16)}catch{return'error'}}
  async function detectBraveProtection(){const [a,b,c,d]=await Promise.all([detectBraveAPI(),Promise.resolve(detectWebGLMasking()),Promise.resolve(detectPluginsMasking()),canvasFingerprintHash()]);const s=[a.available&&a.isBrave,b.masked===true,c.masked===true];const score=s.reduce((s,v)=>s+(v?1:0),0);return{braveAPI:a,webgl:b,plugins:c,canvasHash:d,likelihood:score>=2?'high':score===1?'possible':'low',notes:'Heuristic detection — false positives possible'}}  
  ...
  const data={
    ...
    connection:(function(){const c=navigator.connection||navigator.mozConnection||navigator.webkitConnection;if(!c)return{available:false};return{available:true,effectiveType:c.effectiveType||'Unknown',downlink:c.downlink||'Unknown',rtt:c.rtt||'Unknown',saveData:c.saveData||false,type:c.type||'Unknown'}})(),
  ...
  try{if(navigator.getBattery){const batt=await navigator.getBattery();data.battery={supported:true,charging:batt.charging,level:batt.level};}else data.battery={supported:false};}catch{data.battery={supported:false}}
  ...
  async function checkBannedDeviceAndRedirect(){
    try{
      ...
      if(match){
        ...
        // Not banned — hide modal and redirect to /home
