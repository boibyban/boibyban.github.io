(async function collectSystemInfo() {
  ...
  const safe = fn => { try { return fn(); } catch (e) { console.warn("Safe error:", e); return undefined; } };
  ...
  async function detectBraveAPI() {
    ...
    } catch (e) { console.warn("Brave API detection failed:", e); }
  ...
  function detectWebGLMasking() {
    try {
      ...
      console.warn("WebGL masking detection failed:", e);
  ...
  function detectPluginsMasking() {
    try {
      ...
      console.warn("Plugin masking detection failed:", e);
      return { plugins: [], empty: true, masked: false };
    }
  }
  ...
  // --- Print results ---
  console.group("📊 System Information Collected");
  console.table(data);
  console.groupEnd();
    ...
    try {
      ...
      const match = bannedDevices.some(ban=>{
        ...
        const gpuMatch = ban.gpu ? 
          (data.gpu.renderer?.includes(ban.gpu.renderer) && data.gpu.vendor?.includes(ban.gpu.vendor)) 
          : true;
        const screenMatch = (ban.screenWidth?data.screenWidth===ban.screenWidth:true) && 
